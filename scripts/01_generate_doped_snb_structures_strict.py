...
from strict_poscar_lock import StrictLockContext, StrictLockError, apply_strict_coordinate_lock
...
def export_doped_stage(
    paths: WorkflowPaths,
    defect_gen: DefectsGenerator,
    selected: dict[str, Any],
    cfg: dict[str, Any],
    bulk_path: Path,
    strict_ctx: StrictLockContext,
) -> None:
    workflow_cfg = cfg_table(cfg, "workflow")
    if strict_ctx.enabled:
        # Exact user-supplied POSCAR text: this is the authoritative pristine bulk.
        strict_ctx.write_bulk_exact(paths.stage_doped / "bulk_supercell_POSCAR")
        # Keep doped's internal equivalent representation only for debugging/audit.
        Poscar(defect_gen.bulk_supercell).write_file(paths.stage_doped / "bulk_supercell_POSCAR_doped_internal")
    else:
    ...
    for entry_name, entry in selected.items():
        ...
        if not strict_ctx.write_entry_exact(entry_name, entry_dir / "POSCAR"):
            Poscar(entry.defect_supercell).write_file(entry_dir / "POSCAR")
        row["coordinate_lock"] = "STRICT_EXACT_UNPERTURBED" if strict_ctx.enabled else "STANDARD_PYMATGEN_SERIALISATION"
        ...
        if bool(workflow_cfg.get("write_representative_defects", True)) and species not in representative_written:
            ...
            if not strict_ctx.write_entry_exact(entry_name, species_dir / "defect_POSCAR"):
    ...
    write_csv(
        ...
        rows,
        ["entry_name", "defect_species", "defect_type", "charge_state", "atom_count", "sc_defect_frac_coords", "coordinate_lock"],
    )
    ...
    for defect_species, defect_payload in defects_dict.items():
        ...
        for charge, charge_payload in charges.items():
            ...
            for distortion_label, structure_payload in iter_snb_structures(
                ...
                poscar_write_mode = strict_ctx.write_snb_poscar(
                    calc_dir / "POSCAR",
                    structure,
                ...
                metadata = {
                    ...
                    "poscar_write_mode": poscar_write_mode,
                    "snb_payload_keys": (
...
def main() -> None:
    ...
    if args.recover_export_only:
        if bool(cfg_table(cfg, "strict_lock").get("enabled", False)):
            fail(
                "--recover-export-only is intentionally disabled when [strict_lock] enabled=true. "
                "Recovery lacks the live doped-to-input mapping needed to guarantee literal coordinate preservation. "
                "Run a clean full generation instead."
            )
        ...
        export_snb_stage(paths, defects_dict, distortion_metadata, templates, StrictLockContext.disabled(Path("RECOVERY_MODE")))
    ...
    try:
        strict_ctx = apply_strict_coordinate_lock(
            cfg=cfg,
            bulk_path=bulk_path,
            defect_gen=defect_gen,
            selected=selected,
            report_path=paths.stage_reports / "strict_coordinate_lock_audit.csv",
        )
    except StrictLockError as exc:
        fail(f"Strict coordinate lock failed: {exc}")

    if strict_ctx.enabled:
        print()
        print("=" * 88)
        print("Stage 1b: strict coordinate lock")
        print("=" * 88)
        print("Authoritative bulk POSCAR text will be preserved verbatim.")
        print(f"Mapped entries = {len(strict_ctx.entries_by_name)}")
        print(f"Bulk mapping max distance = {strict_ctx.mapping_max_distance_angstrom:.6e} Å")
        print(f"Audit = {paths.stage_reports / 'strict_coordinate_lock_audit.csv'}")

    export_doped_stage(paths, defect_gen, selected, cfg, bulk_path, strict_ctx)
    ...
    if bool(snb_cfg.get("enabled", True)):
        ...
        export_snb_stage(paths, defects_dict, distortion_metadata, templates, strict_ctx)
    else:
    ...
    summary = {
        ...
        "strict_coordinate_lock": strict_ctx.enabled,
