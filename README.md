...
核心旨在解决 NEB 计算前期极度繁琐且容易出错的建文件夹、改名、配对、 修改任务名及作业提交工作。
...
在执行本脚本前，请务必确保当前工作目录下存在以下文件，且命名绝对规范：
...
工作逻辑：脚本会在根目录下只创建一个共享的 ini 文件夹，并提交一次计算。随后，为每一个指定的末态单独创建文件夹和 fin 子目录。
...
如果超算因为断电、步数耗尽（OOM / Time Limit）导致某个缺陷的 fin 或 ini 计算失败，或者您需要单独调控某个任务的参数（例如提高 EDIFFG、调整 KPOINTS）：
...
如果您习惯在 Windows 系统（如 Notepad++, VS Code）中编辑此脚本，然后再上传到超算服务器运行，极大可能会遇到如下报错：
